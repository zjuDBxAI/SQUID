...
WORKLOAD_AWARE_OVERLAY_TABLE_PREFIX = "workload_documentblocks_overlay_"
WORKLOAD_AWARE_ACCESS_OVERLAY_TABLE_PREFIX = "workload_documentblocks_access_overlay_"
...
LOGICAL_PATTERN_TABLE = "dynamic_partition_current_logical_patterns"
DAG_NODE_TABLE = "dynamic_partition_current_dag_nodes"
...
def get_overlay_table_name(tenant_id: int) -> str:
    return f"{WORKLOAD_AWARE_OVERLAY_TABLE_PREFIX}tenant_{_sanitize_partition_id(str(int(tenant_id)))}"


def get_access_overlay_table_name(tenant_id: int, partition_id: str) -> str:
    return (
        f"{WORKLOAD_AWARE_ACCESS_OVERLAY_TABLE_PREFIX}"
        f"tenant_{_sanitize_partition_id(str(int(tenant_id)))}_partition_{_sanitize_partition_id(partition_id)}"
    )


def get_access_overlay_group_table_name(tenant_id: int) -> str:
    return f"{WORKLOAD_AWARE_ACCESS_OVERLAY_TABLE_PREFIX}tenant_{_sanitize_partition_id(str(int(tenant_id)))}_group"


def get_access_signature_cover_table_name(signature_id: str) -> str:
    return f"{WORKLOAD_AWARE_ACCESS_OVERLAY_TABLE_PREFIX}cover_{_sanitize_partition_id(signature_id)}"
...
class WorkloadQuery:
    ...
    topk: int
    weight: float  #这里有bug; 这个地方的主体应该是该租户访问的评测，而不是对应具体的query


@dataclass(slots=True)
class ACLLogicalPattern:
    pattern_id: int                     # ACL的编号
    tenant_ids: tuple[int, ...]         #tenant原始顺序
    ordered_tenant_ids: tuple[int, ...]  #排序后的顺序
    document_ids: tuple[int, ...]  #包含的文档
    vector_count: int     #向量block
    document_count: int   #文档数量
    entry_tenant_ids: tuple[int, ...] = () #DAG图进入
    metadata: dict[str, object] = field(default_factory=dict) # 存当前ACL块的属性


@dataclass(slots=True)
class PersistedLogicalPattern:
    pattern_id: int
    partition_id: str       # 这个ACL去了哪个分区
    tenant_ids: tuple[int, ...]
...
@dataclass(slots=True)
class PersistedDagNode:
    node_id: int
    prefix_tenants: tuple[int, ...]
    children: dict[int, int] = field(default_factory=dict)
    terminal_pattern_ids: tuple[int, ...] = ()
    supplemental_pattern_ids: tuple[int, ...] = ()
    document_count: int = 0
    terminal_document_count: int = 0
    metadata: dict[str, object] = field(default_factory=dict)
