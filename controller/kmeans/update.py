...
def prepare_kmeans_update_schema(*, db_connection_factory=None) -> None:
    """Run update-schema migrations before admitting concurrent queries."""
    initialize_schema(
        db_connection_factory=db_connection_factory or _default_db_connection_factory
    )


def _acl_key(tenant_ids: Iterable[int]) -> str:
    canonical = ",".join(
        str(int(value)) for value in sorted({int(value) for value in tenant_ids})
    )
    # ACLs can contain thousands of tenants.  The former canonical text was
    # used as a btree primary key and exceeded PostgreSQL's 2704-byte index-row
    # limit.  Keep a stable fixed-width identity instead.
...
_DELETE_OPERATIONS = frozenset({"delete", "remove"})
_INSERT_OPERATIONS = frozenset({"insert", "upsert"})
_VECTOR_UPDATE_OPERATIONS = frozenset({"vector_update", "update_vector"})
_ACL_REPLACE_OPERATIONS = frozenset({"acl_update", "update_acl"})
_ACL_GRANT_OPERATIONS = frozenset({"acl_grant", "grant"})
_ACL_REVOKE_OPERATIONS = frozenset({"acl_revoke", "revoke"})
_SUPPORTED_UPDATE_OPERATIONS = (
    _DELETE_OPERATIONS
    | _INSERT_OPERATIONS
    | _VECTOR_UPDATE_OPERATIONS
    | _ACL_REPLACE_OPERATIONS
    | _ACL_GRANT_OPERATIONS
    | _ACL_REVOKE_OPERATIONS
)


def _validate_update_items(items: Iterable[KMeansUpdateItem]) -> None:
    for item in items:
        operation = str(item.operation).lower()
        if operation not in _SUPPORTED_UPDATE_OPERATIONS:
            raise ValueError(f"Unsupported kmeans update operation: {operation}")
        if operation in (_ACL_GRANT_OPERATIONS | _ACL_REVOKE_OPERATIONS) and not item.role_ids:
            raise ValueError(f"{operation} requires at least one role_id")


def _requires_pre_repair_tombstone(item: KMeansUpdateItem) -> bool:
    # A grant is monotone: it never grants access to an old route.  All other
    # operations may invalidate an old physical copy, so tombstone it before
    # the in-place coverage synchronization.
    return str(item.operation).lower() not in _ACL_GRANT_OPERATIONS
...
class KMeansUpdateRepository:
    ...
    def _collect_tombstone_rows_with_cursor(
        self,
        cur,
        partition_ids: Iterable[str],
        ...
        *,
        ...
        for partition_id in normalized_partitions:
            ...
            cur.execute(
                sql.SQL(
                    """
                    SELECT %s, block_id, document_id, version, pattern_id, %s
                    FROM {}
                    WHERE document_id = ANY(%s);
                    """
    ...
    @staticmethod
    def _write_tombstone_rows_with_cursor(
        cur,
        ...
        execute_values(
            cur,
            f"""
            INSERT INTO {UPDATE_TOMBSTONE_TABLE} (partition_id, block_id, document_id, version, pattern_id, batch_id)
            VALUES %s
            ON CONFLICT (partition_id, block_id, document_id, version)
    ...
    def record_tombstones_for_documents(self, partition_ids: Iterable[str], document_ids: Iterable[int], *, batch_id: int) -> int:
        ...
        try:
            with conn.cursor() as cur:
                rows = self._collect_tombstone_rows_with_cursor(
                    cur,
                    normalized_partitions,
                    normalized_docs,
                    batch_id=int(batch_id),
                )
                count = self._write_tombstone_rows_with_cursor(cur, rows)
            conn.commit()
            return int(count)
        except Exception:
            conn.rollback()
    ...
    def apply_main_table_updates(
        self,
        items: Iterable[KMeansUpdateItem],
        *,
        tombstone_partition_ids: Iterable[str] = (),
        tombstone_document_ids: Iterable[int] = (),
        batch_id: Optional[int] = None,
    ) -> int:
        """Apply tombstones and one main-table update batch atomically.

        When tombstone targets are supplied, their physical row identities are
        read and written on the same connection and committed together with
        the authoritative table changes.  Readers therefore see either the
        complete old state or the complete new main-table/tombstone state.
        """
        normalized_items = tuple(items)
        normalized_partitions = sorted({str(value) for value in tombstone_partition_ids})
        normalized_docs = sorted({int(value) for value in tombstone_document_ids})
        write_tombstones = bool(normalized_partitions and normalized_docs)
        if write_tombstones and batch_id is None:
            raise ValueError("batch_id is required when applying tombstones with main-table updates")
        if write_tombstones:
            initialize_schema(db_connection_factory=self.db_connection_factory)

        role_cache: dict[tuple[int, ...], int] = {}
        conn = self.db_connection_factory()
        try:
            with conn.cursor() as cur:
                tombstone_count = 0
                if write_tombstones:
                    tombstone_rows = self._collect_tombstone_rows_with_cursor(
                        cur,
                        normalized_partitions,
                        normalized_docs,
                        batch_id=int(batch_id),
                    )
                    tombstone_count = self._write_tombstone_rows_with_cursor(cur, tombstone_rows)

                for item in normalized_items:
                    operation = str(item.operation).lower()
                    if operation in _DELETE_OPERATIONS:
                        cur.execute("DELETE FROM PermissionAssignment WHERE document_id = %s;", [int(item.document_id)])
                        cur.execute("DELETE FROM documentblocks WHERE document_id = %s;", [int(item.document_id)])
                        continue
                    if operation in _ACL_GRANT_OPERATIONS:
                        roles = tuple(sorted({int(value) for value in item.role_ids}))
                        cur.execute(
                            """
                            ...
                            WHERE NOT EXISTS (
                                SELECT 1 FROM PermissionAssignment current_assignment
                                WHERE current_assignment.role_id = requested.role_id
                                  AND current_assignment.document_id = %s
                            );
                            """,
                            [int(item.document_id), list(roles), int(item.document_id)],
                        )
                        continue
                    if operation in _ACL_REVOKE_OPERATIONS:
                        cur.execute(
                            "DELETE FROM PermissionAssignment WHERE document_id = %s AND role_id = ANY(%s::BIGINT[]);",
                            [int(item.document_id), list(sorted({int(value) for value in item.role_ids}))],
                        )
                        continue

                    if operation in (_INSERT_OPERATIONS | _VECTOR_UPDATE_OPERATIONS):
                        self._upsert_document_blocks_with_cursor(cur, item)
                    if operation in (_INSERT_OPERATIONS | _VECTOR_UPDATE_OPERATIONS | _ACL_REPLACE_OPERATIONS) and _item_has_acl_payload(item):
                        roles = tuple(sorted({int(value) for value in item.role_ids}))
                        if not roles and item.tenant_ids:
                            roles = (self._ensure_role_for_tenants_with_cursor(cur, item.tenant_ids, role_cache),)
                        cur.execute("DELETE FROM PermissionAssignment WHERE document_id = %s;", [int(item.document_id)])
                        if roles:
                            execute_values(
                                cur,
                                "INSERT INTO PermissionAssignment (role_id, document_id) VALUES %s;",
                                [(int(role_id), int(item.document_id)) for role_id in roles],
                            )
            conn.commit()
            return int(tombstone_count)
        except Exception:
            conn.rollback()
            raise
        finally:
    ...
    def _ensure_role_for_tenants_with_cursor(
        self,
        cur,
        ...
        if tenants in role_cache:
            return int(role_cache[tenants])
        ...
        if row is None:
            cur.execute(
                sql.SQL("SELECT role_id FROM {} WHERE tenant_ids = %s::BIGINT[] LIMIT 1;").format(
            ...
            if row is not None:
                cur.execute(
                    sql.SQL("UPDATE {} SET acl_key = %s WHERE role_id = %s;").format(sql.Identifier(UPDATE_ACL_ROLE_TABLE)),
                    [key, int(row[0])],
                )
        if row is None:
            ...
            execute_values(
                ...
                [(int(tenant_id), role_id) for tenant_id in tenants],
            )
            cur.execute(
                sql.SQL("INSERT INTO {} (acl_key, tenant_ids, role_id) VALUES (%s, %s, %s);").format(sql.Identifier(UPDATE_ACL_ROLE_TABLE)),
                [key, list(tenants), role_id],
            )
        else:
            role_id = int(row[0])
        role_cache[tenants] = int(role_id)
        return int(role_id)

    @staticmethod
    def _upsert_document_blocks_with_cursor(cur, item: KMeansUpdateItem) -> None:
        ...
        if item.source_document_id is not None and not item.vectors:
            ...
            cur.execute(
                ...
                FROM documentblocks WHERE document_id = %s ORDER BY block_id;
                """,
            ...
            execute_values(
                cur,
                "INSERT INTO documentblocks (block_id, document_id, block_content, hash_value, vector) VALUES %s;",
                [(int(block_id), int(item.document_id), b"", b"", list(vector)) for block_id, vector in zip(block_ids, item.vectors)],
            )
    ...
    def ensure_role_for_tenants(self, tenant_ids: Iterable[int]) -> int:
        ...
        try:
            with conn.cursor() as cur:
                ...
                if row is not None:
                    return int(row[0])
                # Lazily migrate pre-versioned rows whose primary key was the
                # full comma-separated tenant list.  This avoids duplicating
                # a role when a restored database already has the ACL mapping.
                cur.execute(
                    sql.SQL("SELECT role_id FROM {} WHERE tenant_ids = %s::BIGINT[] LIMIT 1;").format(
                        sql.Identifier(UPDATE_ACL_ROLE_TABLE)
                    ),
                ...
                if row is not None:
                    cur.execute(
                        sql.SQL("UPDATE {} SET acl_key = %s WHERE role_id = %s;").format(
                            sql.Identifier(UPDATE_ACL_ROLE_TABLE)
                        ),
                        [key, int(row[0])],
                    )
    ...
    def grant_document_roles(self, document_id: int, role_ids: Iterable[int]) -> None:
        """Add role-document permission edges without replacing the existing ACL."""
        roles = tuple(sorted({int(value) for value in role_ids}))
        if not roles:
    ...
    def revoke_document_roles(self, document_id: int, role_ids: Iterable[int]) -> None:
        """Remove only the requested role-document permission edges."""
        roles = tuple(sorted({int(value) for value in role_ids}))
        if not roles:
    ...
    def insert_document_into_partition(
        self,
        ...
        *,
        ...
        """Append a versioned physical copy without retiring older versions.

        This is retained for compatibility with callers outside the update
        pipeline.  New update batches use ``synchronize_existing_partition_documents``
        and pass their batch id as ``version``.
        """
        conn = self.db_connection_factory()
        try:
            with conn.cursor() as cur:
                cur.execute(
                    sql.SQL(
                        ...
                        SELECT block_id, document_id, %s, %s, block_content, vector
                        FROM documentblocks
                        WHERE document_id = %s;
                        """
                    ).format(sql.Identifier(partition.table_name)),
                    [int(pattern_id), int(version), int(document_id)],
                )
            conn.commit()
        finally:
            conn.close()

    def synchronize_existing_partition_documents(
        self,
        *,
        old_partitions: list[KMeansPartition],
        updated_partitions: list[KMeansPartition],
        document_ids: Iterable[int],
        version: int,
    ) -> set[str]:
        """Append changed documents to live partition tables without rebuilding them.

        The caller holds the update/materialization lock and does not admit
        searches until the accompanying route plan has been published.  Only
        existing tables are changed here; genuinely new partitions are built
        separately because no live route can reference them yet.
        """
        touched = sorted({int(document_id) for document_id in document_ids})
        if not touched:
            return set()
        initialize_schema(db_connection_factory=self.db_connection_factory)
        old_by_id = {str(partition.partition_id): partition for partition in old_partitions}
        updated_by_id = {str(partition.partition_id): partition for partition in updated_partitions}
        old_locations = _partition_document_locations(old_partitions, touched)
        updated_locations = _partition_document_locations(updated_partitions, touched)
        existing_ids = set(old_by_id)
        physical_ids = (set(old_locations) | set(updated_locations)) & existing_ids
        if not physical_ids:
            return set()
        ...
        try:
            with conn.cursor() as cur:
                for partition_id in sorted(set(updated_locations) & existing_ids):
                    rows = sorted(
                        (int(document_id), int(pattern_id))
                        for document_id, pattern_id in updated_locations[partition_id].items()
                    )
                    if not rows:
                        continue
                    statement = sql.SQL(
                        """
                        INSERT INTO {} (block_id, document_id, pattern_id, version, block_content, vector)
                        SELECT db.block_id, db.document_id, wanted.pattern_id, {version},
                               db.block_content, db.vector
                        FROM documentblocks db
                        JOIN (VALUES %s) AS wanted(document_id, pattern_id)
                          ON wanted.document_id = db.document_id;
                        """
                    ).format(
                        sql.Identifier(updated_by_id[partition_id].table_name),
                        version=sql.Literal(int(version)),
                    )
                    execute_values(
                        cur,
        ...
        return {
            partition_id
            for partition_id in physical_ids
            if partition_id in updated_by_id
        }


def _apply_main_table_update(repository: KMeansUpdateRepository, item: KMeansUpdateItem) -> None:
    operation = str(item.operation).lower()
    if operation in _DELETE_OPERATIONS:
    ...
    if operation in (_INSERT_OPERATIONS | _VECTOR_UPDATE_OPERATIONS | _ACL_REPLACE_OPERATIONS):
        if _item_has_acl_payload(item):
            repository.replace_document_acl(
...
def apply_main_table_update_batch(
    updates: Iterable[KMeansUpdateItem | dict[str, object]],
    *,
    db_connection_factory=None,
) -> int:
    """Apply a workload batch to authoritative tables without route repair."""
    items = [
        item
        if isinstance(item, KMeansUpdateItem)
        else KMeansUpdateItem.from_mapping(dict(item))
        for item in updates
    ]
    if not items:
        raise ValueError("update batch is empty")
    _validate_update_items(items)
    repository = KMeansUpdateRepository(
        db_connection_factory=db_connection_factory or _default_db_connection_factory
    )
    repository.apply_main_table_updates(items)
...
class _LocalPlanner:
    ...
    def cost(self, partition: KMeansPartition, patterns_by_id: dict[int, ACLPattern]) -> float:
        ...
        total = 0.0
        for _tenant_id, pattern_ids in _tenant_patterns_for_partition(partition, patterns_by_id).items():
    ...
    def selectivity_candidates(
        ...
        result: list[KMeansMaintenanceCandidate] = []
        ranked_partitions: list[tuple[float, float, int, str, KMeansPartition, dict[str, object]]] = []
        for partition in partitions:
        ...
        ranked_partitions.sort(
            key=lambda item: (float(item[0]), float(item[1]), int(item[2]), str(item[3]))
        )
        for _avg_selectivity, _worst_selectivity, _negative_vectors, _partition_id, partition, profile in ranked_partitions:
            candidate, _reason = self._replicated_selectivity_refine_candidate(
                partition,
                patterns_by_id,
                profile,
            )
            if candidate is not None and float(candidate.delta_latency) < 0.0:
    ...
    def _route_aware_cost(
        ...
        total = 0.0
        for by_partition in route_options.values():
    ...
    def _replicated_selectivity_refine_candidate(
        ...
        for tenant_id, pattern_ids in tenant_patterns.items():
            kept_patterns = {
                int(pattern_id)
                for pattern_id in pattern_ids
                if int(pattern_id) in stored_patterns
            }
            ...
            if int(accessible) <= 0:
                continue
            selectivity = (
                float(min(int(accessible), int(partition_vectors)))
                / float(partition_vectors)
            )
            ...
            tenant_rank.append(
                (-float(importance), float(selectivity), int(tenant_id), int(accessible))
            )
        tenant_rank.sort()
        important_tenants = [
            int(tenant_id)
            for _negative_importance, _selectivity, tenant_id, _accessible in tenant_rank[:64]
        ]
        ...
        def weighted_overlap(left_tenant: int, right_tenant: int) -> int:
            return sum(
                ...
                for pattern_id in (
                    set(tenant_patterns[left_tenant])
                    & set(tenant_patterns[right_tenant])
                    & stored_patterns
                )
            )
        ...
        for left_tenant in important_tenants:
            left_seed_patterns = set(tenant_patterns[int(left_tenant)]) & stored_patterns
            if not left_seed_patterns:
                continue
            for right_tenant in important_tenants:
                if int(left_tenant) >= int(right_tenant):
                    continue
                right_seed_patterns = set(tenant_patterns[int(right_tenant)]) & stored_patterns
                if not right_seed_patterns:
                    continue
                union_weight = sum(
                    pattern_weight(pattern_id)
                    for pattern_id in left_seed_patterns | right_seed_patterns
                )
                ...
                rank = (
                    ...
                    int(overlap_weight),
                    -float(
                        tenant_importance.get(int(left_tenant), 0.0)
                        + tenant_importance.get(int(right_tenant), 0.0)
                    ),
                    int(left_tenant),
                    int(right_tenant),
                )
        ...
        for tenant_id in important_tenants:
            ...
            left_overlap = sum(
                pattern_weight(pattern_id)
                for pattern_id in current_patterns & left_union
            )
            right_overlap = sum(
                pattern_weight(pattern_id)
                for pattern_id in current_patterns & right_union
            )
            if int(left_overlap) > int(right_overlap):
                ...
                left_importance = sum(
                    float(tenant_importance.get(int(value), 0.0))
                    for value in left_tenants
                )
                right_importance = sum(
                    float(tenant_importance.get(int(value), 0.0))
                    for value in right_tenants
                )
                if float(left_importance) <= float(right_importance):
                    left_tenants.add(int(tenant_id))
                    left_union.update(current_patterns)
                else:
                    right_tenants.add(int(tenant_id))
        ...
        tenant_side: dict[int, str] = {
            int(tenant_id): "left" for tenant_id in left_tenants
        }
        tenant_side.update(
            {int(tenant_id): "right" for tenant_id in right_tenants}
        )
        for tenant_id, pattern_ids in tenant_patterns.items():
        ...
        for pattern_id in sorted(stored_patterns):
            ...
            left_score = sum(
                float(tenant_importance.get(int(tenant_id), 0.0))
                for tenant_id in pattern_tenants & left_tenants
            )
            right_score = sum(
                float(tenant_importance.get(int(tenant_id), 0.0))
                for tenant_id in pattern_tenants & right_tenants
            )
        ...
        def route_map(pattern_ids: set[int], side: str) -> dict[int, set[int]]:
            ...
            for pattern_id in pattern_ids:
                ...
                for tenant_id in pattern.tenant_ids:
                    if (
                        is_replicated
                        and tenant_side.get(int(tenant_id), str(side)) != str(side)
                    ):
                        continue
                    result[int(tenant_id)].add(int(pattern_id))
            return dict(result)
        ...
        delta_memory = (
            sum(int(value.vector_count) for value in new_partitions)
            - int(partition.vector_count)
        )
        ...
        return (
            KMeansMaintenanceCandidate(
                ...
                payload={
                    ...
                    "replicated_vector_count": int(
                        sum(
                            pattern_weight(pattern_id)
                            for pattern_id in replicated_patterns
                        )
                    ),
        ...
        *,
        ...
        def best_selectivity_candidate() -> tuple[KMeansMaintenanceCandidate | None, dict[str, int | float | str]]:
            ...
            while selectivity_heap:
                ...
                if profile is None:
                    continue
                candidate, reason = self._replicated_selectivity_refine_candidate(
                    partition,
                    patterns_by_id,
                    profile,
                )
        ...
        while True:
            ...
            if str(candidate.op_type) == "replicated_selectivity_split":
    ...
    def plan_local_maintenance(
        ...
        *,
        ...
        while int(memory_now) > int(self.memory_budget):
            compact_candidates = self.compact_candidates([partition for partition in current if str(partition.partition_id) in compact_ids], local_tombstones)
            selectivity_candidates = self.selectivity_candidates([partition for partition in current if str(partition.partition_id) in optimization_ids], patterns_by_id)
            ...
            pair_candidate = None
            if compact_candidates:
                candidates = list(compact_candidates)
                candidates.extend(selectivity_candidates)
        ...
        for state in (memory_pair_state,):
            if state is None:
                continue
...
class _CoreStarPairState:
    def __init__(
        ...
        *,
        ...
        self.operation_rank = {
            "replicated_selectivity_split": 0,
            "split_overlap": 1,
            "merge_extract_overlap": 2,
            "move_left": 3,
            "move_right": 4,
            "full": 5,
        }
    ...
    def _partition_cost(self, partition: KMeansPartition) -> float:
        ...
        value = float(self.planner.cost(partition, self.patterns_by_id))
    ...
    def _best_pair_candidate(self, left: KMeansPartition, right: KMeansPartition) -> KMeansMaintenanceCandidate | None:
        ...
        def spec_cost_memory(pattern_ids: set[int], tenant_patterns: dict[int, set[int]]) -> tuple[float, int, bool]:
            normalized_pattern_ids = {
                int(pattern_id)
                for pattern_id in pattern_ids
                if int(pattern_id) in self.patterns_by_id and int(self.patterns_by_id[int(pattern_id)].vector_count) > 0
            }
            if not normalized_pattern_ids:
                return 0.0, 0, False
            partition_vectors = _partition_live_vectors(normalized_pattern_ids, self.patterns_by_id)
            if partition_vectors <= 0:
                return 0.0, 0, False
            total = 0.0
            live_tenant = False
            for tenant_id, pattern_values in tenant_patterns.items():
                accessible_patterns = {int(pattern_id) for pattern_id in pattern_values if int(pattern_id) in normalized_pattern_ids}
                accessible = _partition_live_vectors(accessible_patterns, self.patterns_by_id)
                if accessible <= 0:
                    continue
                live_tenant = True
                total += estimate_partition_query_cost(
        ...
        best: tuple[tuple[float, int, int], str, float, int, int, list[tuple[set[int], dict[int, set[int]]]]] | None = None
        ...
        for operation in operations:
            specs = self.planner._operation_specs(left, right, self.patterns_by_id, operation)
            after_cost = 0.0
            after_memory = 0
            max_partition_size = 0
            live_count = 0
            for pattern_ids, tenant_patterns in specs:
                spec_cost, spec_memory, live = spec_cost_memory(set(pattern_ids), tenant_patterns)
                if live:
...
def _partition_document_locations(
    partitions: Iterable[KMeansPartition],
    document_ids: Iterable[int],
) -> dict[str, dict[int, int]]:
    """Return the physical copies of selected documents in a partition plan."""
    ...
    for partition in partitions:
        document_patterns = {
            int(document_id): int(pattern_id)
            for document_id, pattern_id in partition.document_pattern_pairs
            if int(document_id) in wanted
        }
        if document_patterns:
            locations[str(partition.partition_id)] = document_patterns
    return locations


def _incremental_coverage_partition_ids(
    old_partitions: list[KMeansPartition],
    updated_partitions: list[KMeansPartition],
    document_ids: Iterable[int],
) -> tuple[set[str], set[str]]:
    """Return (existing changed partitions, newly materialized partitions)."""
    old_ids = {str(partition.partition_id) for partition in old_partitions}
    updated_ids = {str(partition.partition_id) for partition in updated_partitions}
    old_locations = _partition_document_locations(old_partitions, document_ids)
    updated_locations = _partition_document_locations(updated_partitions, document_ids)
    existing_changed = (set(old_locations) | set(updated_locations)) & old_ids & updated_ids
    new_partitions = set(updated_locations) - old_ids
    return existing_changed, new_partitions
...
def _assign_new_patterns(
    ...
    *,
    ...
    # Keep the initial placement policy identical to the batch-maintenance
    # implementation: choose at most two (configurable) existing partitions,
    # prioritizing the choices preferred by the largest number of tenants.
    # ``memory_budget`` remains part of the interface for compatibility, but
    # placement itself does not reject an existing partition on that basis;
    # the maintenance phase is responsible for restoring the budget.
    del memory_budget
    changed: set[str] = set()
    current = list(partitions)
    ...
    for pattern_id in sorted(new_pattern_ids):
        ...
        partitions_by_id = {str(partition.partition_id): partition for partition in current}
        tenant_partition_ids: dict[int, list[str]] = defaultdict(list)
        tenant_patterns_by_partition = {
            partition_id: _tenant_patterns_for_partition(partition, patterns_by_id)
        ...
        def tenant_partition_rank(tenant_id: int, partition_id: str) -> tuple[float, int, str]:
            partition = partitions_by_id[str(partition_id)]
            tenant_patterns = tenant_patterns_by_partition[str(partition_id)]
            before_pattern_ids = set(tenant_patterns.get(int(tenant_id), set()))
            after_pattern_ids = set(before_pattern_ids)
            after_pattern_ids.add(int(pattern_id))
            after_partition_ids = set(map(int, partition.pattern_ids))
            after_partition_ids.add(int(pattern_id))
            before_accessible = _partition_live_vectors(before_pattern_ids, patterns_by_id)
            after_accessible = _partition_live_vectors(after_pattern_ids, patterns_by_id)
            before_vectors = _partition_live_vectors(partition.pattern_ids, patterns_by_id)
            after_vectors = _partition_live_vectors(after_partition_ids, patterns_by_id)
            delta_cost = (
                route_cost(after_vectors, after_accessible)
                - route_cost(before_vectors, before_accessible)
            )
        ...
        for tenant_id in sorted(pattern_tenants):
            choices = sorted(
                (
                    tenant_partition_rank(int(tenant_id), str(partition_id)),
                    str(partition_id),
                )
            ...
            if choices:
                ...
                grouped_best_tenants[best_partition_id].add(int(tenant_id))
                grouped_best_cost[best_partition_id] += float(best_rank[0])

        selected_partition_ids = [
            partition_id
            for partition_id, tenants in sorted(
                grouped_best_tenants.items(),
                key=lambda item: (
                    -len(item[1]),
                    float(grouped_best_cost.get(item[0], 0.0)),
                    tenant_partition_rank(next(iter(item[1])), item[0])[1],
                    item[0],
                ),
            )[:max_links]
        ]
        ...
        for tenant_id in sorted(pattern_tenants):
            if selected_partition_ids:
                best_partition_id = min(
                    selected_partition_ids,
                    key=lambda partition_id: tenant_partition_rank(
                        int(tenant_id), str(partition_id)
                    ),
                )
                selected_tenants_by_partition[best_partition_id].add(int(tenant_id))

        # Match the old fallback: if no tenant has an existing candidate,
        # prefer the smallest resulting existing partition before creating a
        # brand-new private partition.
        if not selected_tenants_by_partition and current:
            best_partition = min(
                current,
                key=lambda partition: (
                    _partition_live_vectors(
                        set(map(int, partition.pattern_ids)) | {int(pattern_id)},
                        patterns_by_id,
                    ),
                    len(partition.pattern_ids),
                    str(partition.partition_id),
                ),
            )
            selected_tenants_by_partition[str(best_partition.partition_id)] = set(
                pattern_tenants
            )

        updated: list[KMeansPartition] = []
        assigned_existing_count = 0
        for partition in current:
            assigned_tenants = selected_tenants_by_partition.get(
                str(partition.partition_id), set()
            )
            ...
            rebuilt = _make_partition(
                str(partition.partition_id),
                cluster_id=int(partition.cluster_id),
                pattern_ids=set(partition.pattern_ids) | {int(pattern_id)},
                tenant_patterns=tenant_patterns,
                patterns_by_id=patterns_by_id,
            )
            if rebuilt is not None:
        ...
        if not assigned_existing_count:
            ...
            tenant_patterns = {int(tenant_id): {int(pattern_id)} for tenant_id in pattern.tenant_ids}
...
def _validate_route_coverage(
    ...
    patterns_by_id = {int(pattern.pattern_id): pattern for pattern in plan.patterns}
    requirements = _tenant_requirements(patterns_by_id)
    partitions_by_id = {str(partition.partition_id): partition for partition in plan.partitions}
    ...
    for route in plan.tenant_routes:
        ...
        if partition is None:
            raise RuntimeError(
                f"Route coverage validation failed: route for tenant {tenant_id} "
                f"references missing partition {route.partition_id}"
            )
        partition_patterns = set(map(int, partition.pattern_ids))
        route_patterns = set(map(int, route.pattern_ids))
        invalid_patterns = route_patterns - partition_patterns
        if invalid_patterns:
            raise RuntimeError(
                f"Route coverage validation failed: route {route.partition_id} for tenant {tenant_id} "
                f"contains patterns not stored by the partition: {sorted(invalid_patterns)}"
            )
        unauthorized_patterns = {
            int(pattern_id)
            for pattern_id in route_patterns
            if int(pattern_id) not in patterns_by_id
            or int(tenant_id) not in set(map(int, patterns_by_id[int(pattern_id)].tenant_ids))
        }
        if unauthorized_patterns:
            raise RuntimeError(
                f"Route authorization validation failed for tenant {tenant_id}: "
                f"{sorted(unauthorized_patterns)}"
            )
        covered[int(tenant_id)].update(route_patterns)
        route_count += 1
    for tenant_id, required_patterns in requirements.items():
        missing = set(required_patterns) - covered.get(int(tenant_id), set())
        if missing:
            raise RuntimeError(
                f"Route coverage validation failed for tenant {tenant_id}: missing patterns {sorted(missing)}"
            )
    return {
        "validated_tenant_count": int(len(requirements)),
        "validated_route_count": int(route_count),
        "validated_partition_count": int(len(plan.partitions)),
    }


def _update_shadow_partition_id(batch_id: int, phase: str, partition_id: str) -> str:
    phase_token = "".join(char for char in str(phase).lower() if char.isalnum())[:1] or "u"
    digest = hashlib.blake2b(
        f"{int(batch_id)}:{phase_token}:{partition_id}".encode("utf-8"),
        digest_size=8,
    ).hexdigest()
    # Keep the derived physical table name safely below PostgreSQL's 63-byte
    # identifier limit while retaining the full collision-resistant digest.
    return f"u_{phase_token}_{digest}"


def _shadow_changed_partitions(
    plan: KMeansPlan,
    *,
    changed_partition_ids: set[str],
    batch_id: int,
    phase: str,
) -> tuple[KMeansPlan, dict[str, str], set[str]]:
    target_ids = {str(value) for value in changed_partition_ids}
    id_map = {
        str(partition.partition_id): _update_shadow_partition_id(
            int(batch_id),
            str(phase),
            str(partition.partition_id),
        )
        for partition in plan.partitions
        if str(partition.partition_id) in target_ids
    }
    shadow_partitions: list[KMeansPartition] = []
    for partition in plan.partitions:
        old_id = str(partition.partition_id)
        new_id = id_map.get(old_id, old_id)
        table_name = get_partition_table_name(new_id) if old_id in id_map else str(partition.table_name)
        metadata = dict(partition.metadata or {})
        if old_id in id_map:
            metadata.update(
                {
                    "update_shadow_batch_id": int(batch_id),
                    "update_shadow_phase": str(phase),
                    "update_shadow_source_partition_id": str(old_id),
                }
            )
        shadow_partitions.append(
            KMeansPartition(
                partition_id=str(new_id),
                cluster_id=int(partition.cluster_id),
                partition_kind=str(partition.partition_kind),
    ...
    shadow_routes = [
        TenantRoute(
            tenant_id=int(route.tenant_id),
            partition_id=id_map.get(str(route.partition_id), str(route.partition_id)),
            table_name=(
                get_partition_table_name(id_map[str(route.partition_id)])
                if str(route.partition_id) in id_map
                else str(route.table_name)
            ),
            route_kind=str(route.route_kind),
            cluster_id=int(route.cluster_id),
            pattern_ids=route.pattern_ids,
            partition_vector_count=int(route.partition_vector_count),
            accessible_vector_count=int(route.accessible_vector_count),
        )
        for route in plan.tenant_routes
    ]
    metadata = {
    ...
    return (
        KMeansPlan(
            ...
            tenant_routes=shadow_routes,
            tenant_to_cluster=dict(plan.tenant_to_cluster),
...
def _validate_materialized_plan(
    plan: KMeansPlan,
    *,
    staged_partition_ids: set[str],
    repository: KMeansUpdateRepository,
) -> dict[str, int]:
    staged_ids = {str(value) for value in staged_partition_ids}
    patterns_by_id = {int(pattern.pattern_id): pattern for pattern in plan.patterns}
    conn = repository.db_connection_factory()
    validated_rows = 0
    try:
        with conn.cursor() as cur:
            for partition in plan.partitions:
                cur.execute("SELECT to_regclass(%s);", [str(partition.table_name)])
                if cur.fetchone()[0] is None:
                    raise RuntimeError(
                        f"Physical coverage validation failed: table {partition.table_name} does not exist"
                    )
                if str(partition.partition_id) not in staged_ids:
                    continue
                cur.execute(
                    sql.SQL(
                        """
                        SELECT p.pattern_id, COUNT(*)::BIGINT
                        FROM {} p
                        ...
                        sql.Identifier(str(partition.table_name)),
                        sql.Identifier(UPDATE_TOMBSTONE_TABLE),
                    ),
                    [list(map(int, partition.pattern_ids)), str(partition.partition_id)],
                )
                actual_by_pattern = {int(pattern_id): int(count) for pattern_id, count in cur.fetchall()}
                expected_by_pattern = {
                    int(pattern_id): int(patterns_by_id[int(pattern_id)].vector_count)
                    for pattern_id in partition.pattern_ids
                    if int(pattern_id) in patterns_by_id
                }
                if actual_by_pattern != expected_by_pattern:
                    raise RuntimeError(
                        f"Physical coverage validation failed for {partition.table_name}: "
                        f"expected pattern counts {expected_by_pattern}, got {actual_by_pattern}"
                    )
                validated_rows += int(sum(actual_by_pattern.values()))
    finally:
        conn.close()
    return {
        "validated_physical_partition_count": int(len(staged_ids)),
        "validated_physical_vector_count": int(validated_rows),
    }


def _materialize_validate_and_publish(
    plan: KMeansPlan,
    *,
    staged_partition_ids: set[str],
    create_indexes: bool,
    ...
    validation_metadata = _validate_route_coverage(plan)
    validation_metadata["physical_validation_enabled"] = int(bool(validate_physical))
    if validate_physical:
        validation_metadata.update(
            _validate_materialized_plan(
                plan,
                staged_partition_ids=staged_ids,
                repository=repository,
            )
        )
    # This transaction is the publication point. Every table referenced by the
    # new routes exists; expensive full physical validation is opt-in.
    save_plan(plan, db_connection_factory=repository.db_connection_factory)
    invalidate_cache()
    return validation_metadata


def _apply_kmeans_update_batch_locked(
    ...
    *,
    ...
    validate_physical: bool = False,
    db_connection_factory=None,
    main_table_applied_callback: Optional[Callable[[int], None]] = None,
    repair_published_callback: Optional[Callable[[KMeansPlan], None]] = None,
) -> KMeansUpdateResult:
    ...
    tombstone_docs = {
        int(item.document_id)
        for item in items
        if _requires_pre_repair_tombstone(item)
    }
    tombstone_partition_ids = {
        partition_id
        for document_id in tombstone_docs
        for partition_id in old_doc_partitions.get(int(document_id), ())
    }
    apply_started_at = time.perf_counter()
    tombstone_count = repository.apply_main_table_updates(
        items,
        tombstone_partition_ids=tombstone_partition_ids,
        tombstone_document_ids=tombstone_docs,
        batch_id=int(batch_id),
    )
    ...
    if main_table_applied_callback is not None:
        main_table_applied_callback(int(batch_id))
    ...
    phase_started_at = time.perf_counter()
    refreshed_partitions = _rebuild_partitions_with_patterns(old_partitions, patterns_by_id)
    memory_budget = _memory_budget(plan_summary, refreshed_patterns)
    top_d = _private_edge_top_d(plan_summary)
    refreshed_partitions, new_assignment_partitions = _assign_new_patterns(
    ...
    refreshed_partitions, repair_routes, pre_maintenance_route_repair_metadata = _repair_routes_greedy(
        refreshed_partitions,
        patterns_by_id,
        affected_tenant_ids=affected_tenant_ids,
        existing_routes=old_routes,
    )
    repair_validation_metadata = _validate_route_coverage(
        KMeansPlan(
            partitions=refreshed_partitions,
            tenant_routes=repair_routes,
            tenant_to_cluster={int(route.tenant_id): int(route.cluster_id) for route in repair_routes},
            patterns=refreshed_patterns,
        )
    )
    timing["pre_maintenance_route_repair_seconds"] = time.perf_counter() - phase_started_at
    ...
    # Coverage is an in-place, quiesced update.  Rewrite only the changed
    # documents in existing partition tables; do not rebuild every partition
    # containing their ACL pattern.  A partition created by a new assignment
    # has no old readers, so it is the sole case that is materialized from
    # scratch before publication.
    in_place_partition_ids, new_partition_ids = _incremental_coverage_partition_ids(
        old_partitions,
        refreshed_partitions,
        touched_docs,
    )
    repair_plan_metadata = {
        **dict(plan_summary.get("metadata", {}) or {}),
        ...
        "update_publish_phase": "coverage_repair",
        "coverage_in_place_partition_count": int(len(in_place_partition_ids)),
        "coverage_new_partition_count": int(len(new_partition_ids)),
        "partition_count": int(len(refreshed_partitions)),
        ...
        "partition_vector_count": int(sum(int(partition.vector_count) for partition in refreshed_partitions)),
        **pre_maintenance_route_repair_metadata,
        **repair_validation_metadata,
    }
    repair_plan = KMeansPlan(
        partitions=refreshed_partitions,
        tenant_routes=repair_routes,
        tenant_to_cluster={int(route.tenant_id): int(route.cluster_id) for route in repair_routes},
        patterns=refreshed_patterns,
    ...
    partitions_by_id = {str(partition.partition_id): partition for partition in repair_plan.partitions}
    coverage_new_partition_started_at = time.perf_counter()
    for partition_id in sorted(new_partition_ids):
        partition = partitions_by_id[str(partition_id)]
        materialize_partition(partition, db_connection_factory=repository.db_connection_factory)
        if create_indexes:
            create_index_for_partition(
    ...
    timing["coverage_new_partition_materialization_seconds"] = (
        time.perf_counter() - coverage_new_partition_started_at
    )
    coverage_existing_sync_started_at = time.perf_counter()
    synchronized_partition_ids = repository.synchronize_existing_partition_documents(
        old_partitions=old_partitions,
        updated_partitions=repair_plan.partitions,
        document_ids=touched_docs,
        version=int(batch_id),
    )
    timing["coverage_existing_partition_sync_seconds"] = (
        time.perf_counter() - coverage_existing_sync_started_at
    )
    repair_staged_partition_ids = set(new_partition_ids) | set(synchronized_partition_ids)
    coverage_validation_started_at = time.perf_counter()
    repair_publish_validation_metadata = _validate_route_coverage(repair_plan)
    repair_publish_validation_metadata["physical_validation_enabled"] = int(bool(validate_physical))
    if validate_physical:
        repair_publish_validation_metadata.update(
            _validate_materialized_plan(
                repair_plan,
                staged_partition_ids=repair_staged_partition_ids,
                repository=repository,
            )
        )
    timing["coverage_publish_validation_seconds"] = (
        time.perf_counter() - coverage_validation_started_at
    )
    # Existing tables retain old versions.  Tombstones hide only those exact
    # versions; the newly appended batch version is queryable immediately.
    coverage_plan_save_started_at = time.perf_counter()
    save_plan(repair_plan, db_connection_factory=repository.db_connection_factory)
    timing["coverage_plan_save_seconds"] = (
        time.perf_counter() - coverage_plan_save_started_at
    )
    invalidate_cache()
    repair_id_map = {str(partition.partition_id): str(partition.partition_id) for partition in repair_plan.partitions}
    timing["coverage_repair_publish_seconds"] = time.perf_counter() - phase_started_at
    repair_plan.metadata.update(repair_publish_validation_metadata)
    repair_plan.metadata.update(timing)
    print(
        f"[kmeans-update] batch {batch_id}: coverage repair published after "
        f"incrementally updating {len(synchronized_partition_ids)} existing partitions "
        f"and materializing {len(new_partition_ids)} new partitions",
        flush=True,
    )
    if repair_published_callback is not None:
        callback_started_at = time.perf_counter()
        repair_published_callback(repair_plan)
        timing["repair_published_callback_seconds"] = time.perf_counter() - callback_started_at
    print(f"[kmeans-update] batch {batch_id}: maintenance starts now", flush=True)

    # From this point onward queries use the physically complete repair plan.
    # Maintenance works on that published snapshot and never mutates its tables.
    refreshed_partitions = list(repair_plan.partitions)
    old_routes = list(repair_plan.tenant_routes)
    query_direct_partition_ids = {
        repair_id_map.get(str(partition_id), str(partition_id))
        for partition_id in query_direct_partition_ids
    }
    phase_started_at = time.perf_counter()
    ...
    # Old versions remain physically present until their tombstone ratio reaches
    # the compaction threshold in background maintenance.
    ...
    compact_ids = set(global_compact_ids)
    memory_pair_seed_ids = (
        set(compact_ids)
        | set(tombstone_partition_ids)
        | set(new_assignment_partitions)
    )
    ...
    memory_pair_region_metadata = {
        "memory_pair_seed_partition_count": int(len(memory_pair_seed_ids)),
        "memory_pair_region_partition_count": int(len(memory_pair_region_ids)),
        "memory_pair_region_core_star_edges": int(
            memory_pair_region_raw_metadata.get("affected_core_star_edges", 0)
        ),
        "memory_pair_region_owner_patterns": int(
            memory_pair_region_raw_metadata.get(
                "affected_core_star_owner_patterns", 0
            )
        ),
    }
    memory_now = sum(
        int(partition.vector_count)
        + int(tombstones_by_partition.get(str(partition.partition_id), 0))
        for partition in refreshed_partitions
    )
    selection_mode = (
        "memory_reduction"
        if int(memory_now) > int(memory_budget)
        else "query_cost_reduction"
    )
    affected_ids = set(query_affected_ids) | set(compact_ids)
    maintenance_scope_ids = set(affected_ids)
    if selection_mode == "memory_reduction":
    ...
    affected_partitions = [
        partition
        for partition in refreshed_partitions
        if str(partition.partition_id) in maintenance_scope_ids
    ]
    ...
    timing["post_maintenance_route_repair_seconds"] = time.perf_counter() - phase_started_at
    print(
        f"[kmeans-update] batch {batch_id}: post-maintenance route validation covers "
        f"{route_repair_metadata.get('route_repair_route_count', 0)} routes",
        flush=True,
    )
    ...
    metadata.update(
        {
            ...
            "update_publish_phase": "maintenance",
            **affected_region_metadata,
            **maintenance_selection_metadata,
            **pre_maintenance_route_repair_metadata,
            **repair_publish_validation_metadata,
    ...
    maintenance_validation_metadata = _validate_route_coverage(maintenance_plan)
    maintenance_staged_partition_ids: set[str] = set()
    if accepted:
        maintenance_plan, _maintenance_id_map, maintenance_staged_partition_ids = _shadow_changed_partitions(
            maintenance_plan,
            changed_partition_ids=rewritten_partition_ids,
            batch_id=int(batch_id),
            phase="maintenance",
        )
        maintenance_validation_metadata.update(
            _materialize_validate_and_publish(
                maintenance_plan,
                staged_partition_ids=maintenance_staged_partition_ids,
                create_indexes=bool(create_indexes),
                index_type=str(index_type),
                repository=repository,
                validate_physical=bool(validate_physical),
            )
        )
        # Keep tombstones for the superseded table while pre-publication queries
        # may still be in flight.  Safe epoch/grace-period cleanup is separate
        # from publication; clearing here could expose deleted rows to an old
        # route snapshot.
        print(
            f"[kmeans-update] batch {batch_id}: maintenance published with "
            f"{len(maintenance_staged_partition_ids)} shadow partitions",
            flush=True,
        )
    timing["maintenance_publish_seconds"] = time.perf_counter() - phase_started_at
    all_rewritten_partition_ids = set(repair_staged_partition_ids) | set(maintenance_staged_partition_ids)
    ...
    return KMeansUpdateResult(
        ...
        rewritten_partition_ids=tuple(sorted(all_rewritten_partition_ids)),
        ...
        metadata={
            ...
            **repair_publish_validation_metadata,
            **route_repair_metadata,
            **maintenance_validation_metadata,
            **timing,
            "memory_budget": int(memory_budget),
...
def apply_kmeans_update_batch(
    ...
    *,
    ...
    connection_factory = db_connection_factory or _default_db_connection_factory
    lock_conn = _acquire_materialize_lock(db_connection_factory=connection_factory)
    try:
        return _apply_kmeans_update_batch_locked(
            updates,
            tau_del=float(tau_del),
            max_operations=int(max_operations),
            max_new_pattern_partitions=int(max_new_pattern_partitions),
            enable_maintenance=bool(enable_maintenance),
            create_indexes=bool(create_indexes),
            index_type=str(index_type),
            validate_physical=bool(validate_physical),
            db_connection_factory=connection_factory,
            main_table_applied_callback=main_table_applied_callback,
            repair_published_callback=repair_published_callback,
        )
    finally:
