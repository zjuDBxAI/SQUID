...
void search_neighbors_to_add(
    ...
    (void)reference_version; // Disabled branch selection optimization; always use reference behavior.
    ...
    while (!candidates.empty()) {
        ...
        if (reference_version) {
            ...
            /* Disabled batched neighbor expansion optimization for benchmarking.
            // a faster version
            ...
            */
            ...
            // Fallback to reference-style processing while optimization is disabled.
            for (size_t i = begin; i < end; i++) {
                storage_idx_t nodeId = hnsw.neighbors[i];
                if (nodeId < 0) {
                    break;
...
int search_from_candidates(
    ...
    while (candidates.size() > 0) {
        ...
        for (size_t j = begin; j < end; j++) {
            ...
            // Disabled L2 prefetch to remove cache optimization overhead.
        ...
        if (hnsw.use_optimized_search) {
            /* Disabled batched search optimization for benchmarking.
            int counter = 0;
            size_t saved_j[4];
            ...
            */
            ...
            // Fallback to sequential processing while optimization is disabled.
            for (size_t j = begin; j < jmax; j++) {
                int v1 = hnsw.neighbors[j];
...
std::priority_queue<HNSW::Node> search_from_candidate_unbounded(
    ...
    while (!candidates.empty()) {
        ...
        if (hnsw.use_optimized_search) {
            /* Disabled optimized batch candidate expansion for benchmarking.
            int counter = 0;
            size_t saved_j[4];
...
HNSWStats greedy_update_nearest(
    ...
    for (;;) {
        ...
        if (hnsw.use_optimized_search) {
            /* Disabled greedy batch optimization for benchmarking.
            int n_buffered = 0;
            storage_idx_t buffered_ids[4];
...
// baseline non-vectorized version kept for experimentation.
int HNSW::MinimaxHeap::pop_min(float* vmin_out) {
    assert(k > 0);
