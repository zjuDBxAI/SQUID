...
// Disabled optimization: reuse of storage-backed distance computer is commented out for profiling.
/*struct RemappedDistanceComputer : DistanceComputer {
    std::unique_ptr<DistanceComputer> base;
    const std::vector<idx_t>& id_map;

    RemappedDistanceComputer(DistanceComputer* base_in, const std::vector<idx_t>& map)
            : base(base_in), id_map(map) {}
    ...
    float symmetric_dis(idx_t i, idx_t j) override {
        return base->symmetric_dis(id_map[i], id_map[j]);
    }
};

DistanceComputer* storage_distance_computer(const IndexHNSW& index) {
    DistanceComputer* base = index.storage->get_distance_computer();
    if (index.use_storage_ids) {
        base = new RemappedDistanceComputer(base, index.storage_ids);
    }
    if (is_similarity_metric(index.storage->metric_type)) {
        return new NegativeDistanceComputer(base);
    }
    return base;
}*/

struct SlowDistanceComputer : DistanceComputer {
    const IndexHNSW& index;
    std::vector<float> buffer_i;
    std::vector<float> buffer_j;
    const float* query = nullptr;
    size_t distance_calls = 0;
    size_t symmetric_calls = 0;

    explicit SlowDistanceComputer(const IndexHNSW& index_in)
            : index(index_in),
              buffer_i(index_in.d, 0.0f),
              buffer_j(index_in.d, 0.0f) {}

    inline idx_t map_id(idx_t local_id) const {
        return index.use_storage_ids ? index.storage_ids[local_id] : local_id;
    }
    ...
    float operator()(idx_t i) override {
        FAISS_ASSERT(query);
        idx_t storage_id = map_id(i);
        index.storage->reconstruct(storage_id, buffer_i.data());
        distance_calls++;
        if (index.metric_type == METRIC_INNER_PRODUCT) {
            return -fvec_inner_product(query, buffer_i.data(), index.d);
        }
        if (index.metric_type == METRIC_L2) {
            return fvec_L2sqr(query, buffer_i.data(), index.d);
        }
        // Disabled fast path: reuse storage distance computer; fallback to generic computation.
        std::unique_ptr<DistanceComputer> fallback(index.storage->get_distance_computer());
        fallback->set_query(query);
        return (*fallback)(storage_id);
    }

    float symmetric_dis(idx_t i, idx_t j) override {
        idx_t storage_i = map_id(i);
        idx_t storage_j = map_id(j);
        index.storage->reconstruct(storage_i, buffer_i.data());
        index.storage->reconstruct(storage_j, buffer_j.data());
        symmetric_calls++;
        if (index.metric_type == METRIC_INNER_PRODUCT) {
            return -fvec_inner_product(buffer_i.data(), buffer_j.data(), index.d);
        }
        if (index.metric_type == METRIC_L2) {
            return fvec_L2sqr(buffer_i.data(), buffer_j.data(), index.d);
        }
        std::unique_ptr<DistanceComputer> fallback(index.storage->get_distance_computer());
        fallback->set_query(buffer_i.data());
        return (*fallback)(storage_j);
    }
};

DistanceComputer* storage_distance_computer(const IndexHNSW& index) {
    // Disabled optimization: fall back to reconstructing vectors on demand instead of reusing storage DC.
    return new SlowDistanceComputer(index);
}
...
#pragma omp parallel if (i1 > i0 + 100)
            {
                ...
                std::unique_ptr<DistanceComputer> dis(
                        storage_distance_computer(index_hnsw));
                int prev_display =
...
#pragma omp parallel if (i1 - i0 > 1)
        {
            ...
            std::unique_ptr<DistanceComputer> dis(
                    storage_distance_computer(*index));
...
#pragma omp for reduction(+ : n1, n2, ndis, nhops) schedule(guided)
            for (idx_t i = i0; i < i1; i++) {
                ...
                // if (auto slow = dynamic_cast<SlowDistanceComputer*>(dis.get())) {
                //     double kb = 0.0;
                //     if (index->d > 0) {
                //         kb = slow->distance_calls * index->d * sizeof(float) / 1024.0;
                //     }
                //     printf("[faiss::IndexHNSW] query %" PRId64 " touched %zu vectors (~%.2f KB) "
                //            "(symmetric=%zu)\n",
                //            i,
                //            slow->distance_calls,
                //            kb,
                //            slow->symmetric_calls);
                // }
                n1 += stats.n1;
                n2 += stats.n2;
                ndis += stats.ndis;
...
void IndexHNSW::add_from_storage_ids(idx_t n, const idx_t* ids) {
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "IndexHNSW requires a backing storage before adding vectors");
    FAISS_THROW_IF_NOT(is_trained);
    ...
    // Ensure mapping vector has entries for the existing nodes. If this index
    // was previously populated via the traditional add() path, create the
    // identity mapping so future distance computations can use storage ids
    // transparently.
    if (!use_storage_ids && ntotal > 0) {
        storage_ids.resize(ntotal);
        for (idx_t i = 0; i < ntotal; ++i) {
            storage_ids[i] = i;
        }
        use_storage_ids = true;
    }

    std::vector<float> contiguous_vectors(n * d);
    for (idx_t i = 0; i < n; ++i) {
        idx_t storage_id = ids[i];
        FAISS_THROW_IF_NOT_MSG(
                storage_id >= 0 && storage_id < storage->ntotal,
                "storage id out of range in add_from_storage_ids");
        storage->reconstruct(storage_id, contiguous_vectors.data() + i * d);
    }

    idx_t n0 = ntotal;
    ntotal += n;

    storage_ids.insert(storage_ids.end(), ids, ids + n);
    use_storage_ids = true;
...
#pragma omp parallel
    {
        std::unique_ptr<DistanceComputer> dis(
                storage_distance_computer(*this));
...
#pragma omp parallel for
    for (idx_t i = 0; i < ntotal; i++) {
        DistanceComputer* qdis = storage_distance_computer(*this);
...
DistanceComputer* IndexHNSW::get_distance_computer() const {
    return storage_distance_computer(*this);
}
