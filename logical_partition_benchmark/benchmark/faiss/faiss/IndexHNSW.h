...
struct IndexHNSW : Index {
    ...
    // Optional mapping from local node ids (0..ntotal-1) to entries that
    // already exist in the shared storage. When empty, ids are assumed to be
    // contiguous and owned by this index (the default Faiss behaviour).
    bool use_storage_ids = false;
    std::vector<idx_t> storage_ids;
    ...
    /// Build only the HNSW graph over vectors that already live in the
    /// underlying storage. No copies are made and the graph reuses the
    /// storage indices supplied in `ids` (one per vector, insertion order
    /// preserved).
    void add_from_storage_ids(idx_t n, const idx_t* ids);
    ...
    void set_use_optimizations(bool enable) {
        hnsw.use_optimized_search = enable;
        hnsw.use_optimized_graph = enable;
    }
};
