...
int main(int argc, char** argv) {
    try {
        std::cout << "========================================" << std::endl;
        std::cout << "Test: Independent HNSW - Role Partition (Physical)" << std::endl;
        std::cout << "========================================\n" << std::endl;
        ...
        const auto role_dir = base_dir / "physical_role_partition";
        ...
        std::unordered_map<int, std::filesystem::path> partition_dirs;
        ...
        for (const auto& table : partitions) {
            ...
            auto partition_dir = role_dir / table;
            ...
            partition_dirs.emplace(role_id, partition_dir);
        }

        const size_t total_bytes = total_vector_bytes + total_graph_bytes;
        ...
        if (!query_roles.empty()) {
            for (int role_id : query_roles) {
                auto dir_it = partition_dirs.find(role_id);
                if (dir_it == partition_dirs.end()) {
                    continue;
                }

                IndependentHNSWIndex index(dimension, M, efConstruction);
                if (!index.load_components(dir_it->second.string())) {
                    std::cerr << "  Warning: failed to load " << dir_it->second
                              << ", rebuilding..." << std::endl;
                    index.build_from_partition(conn_info, "documentblocks_role_" + std::to_string(role_id));
                    index.save_components(dir_it->second.string());
                }
                index.set_ef_search(efSearch);

                for (int warm = 0; warm < warmup_iterations; ++warm) {
                    index.search(query.query_vector.data(), query.topk);
                }
                auto search_start = std::chrono::high_resolution_clock::now();
                auto hits = index.search(query.query_vector.data(), query.topk);
        ...
        query_times.push_back(total_search_time_ms);
        ...
        benchmark_utils::json output = {
            {"strategy", "physical_role_partition"},
            {"storage", {
        ...
        const auto output_path = root / "logical_partition_benchmark/benchmark/src/physical_role_partition_results.json";
