...
int main(int argc, char** argv) {
    try {
        std::cout << "========================================" << std::endl;
        std::cout << "Test: Pointer HNSW - Dynamic Partition" << std::endl;
        std::cout << "========================================\n" << std::endl;
        ...
        const auto dynamic_dir = base_dir / "dynamic_partition";
        ...
        std::unordered_map<std::string, std::unique_ptr<PointerHNSWIndex>> indexes;
        ...
        for (const auto& table : partitions) {
            auto graph_path = dynamic_dir / (table + "_graph.bin");
            ...
            indexes.emplace(table, std::move(index));
        }
        ...
        for (size_t i = 0; i < queries.size(); ++i) {
            ...
            std::vector<std::pair<SharedVectorTable::DocBlockId, double>> candidates;
            ...
            if (!query_roles.empty()) {
                ...
                if (combo_it != combo_to_partitions.end()) {
                    ...
                    for (const auto& partition_name : combo_it->second) {
                        auto idx_it = indexes.find(partition_name);
                        if (idx_it == indexes.end()) {
                            continue;
                        }
                        auto& index = *idx_it->second;

                        size_t partition_size = index.size();
                        if (partition_size == 0) {
                            continue;
                        }
        ...
        const auto output_path = root / "logical_partition_benchmark/benchmark/src/dynamic_partition_results.json";
